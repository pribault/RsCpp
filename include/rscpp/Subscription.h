...
namespace rscpp
{
	...
	public:
		 ...
		 * <p>
		 *  It can be called however often and whenever needed—but if the outstanding cumulative demand ever becomes std::numeric_limits<std::size_t>::max() or
		 * more, it may be treated by the {@link rscpp::Publisher} as "effectively unbounded". <p> Whatever has been requested can be sent by the
		 * {@link rscpp::Publisher} so only signal demand for what can be safely handled. <p> A {@link rscpp::Publisher} can send less than is requested if the
		 * stream ends but then must emit either {@link rscpp::Subscriber::onError(const std::exception_ptr&)} or {@link rscpp::Subscriber::onComplete()}.
		 *
		 * @param count the strictly positive number of elements to requests to the upstream {@link rscpp::Publisher}
		 */
		virtual void request(std::size_t count);
